...
この文書は `docs/tasks/secret-recovery/tasks.md` の作業項目 `<作業項目名>` に対する固定実装単位 `レビュー` の記録先である。
...
- 確認開始時 HEAD: `未記入`
- 対象差分識別子: `未記入`
- 実装側確認記録: `./<作業項目ディレクトリ>/confirmation.md`（このテンプレート複製先の同階層 `confirmation.md` を指定。補助記録であり、exact 同期は gate ではない）

## レビュー担当チェック項目

1. 対象作業項目のスコープ外へ越境していないこと。
2. 責務境界と依存方向が [レビュー観点チェックリスト](../../../architecture/review-checklist.md#レビュー観点チェックリスト構造) の観点に適合していること。
   - 変更対象ファイルのディレクトリ名から所属層を特定し（`adapters/`、`application/`、`domain/`、`ports/`、`support/`、`tests/`）、`review-checklist.md` の対応セクションの全チェック項目を照合すること。
   - `adapters/` 配下のファイルは必ず **公開面（絶対規則）** セクションを適用し、port trait を実装する型・メソッド以外の関数・型・定数が `pub`・`pub(crate)`・`pub(super)` で外部に公開されていないことを確認すること。この確認を省略してはならない。
...
- `未着手`、`判定保留`、`コード差分なし` などの状態説明は `判定` に書かず、`判定要約` と `根拠` に記録する。合否を開始できない段階の標準ラベルは `要修正` とする。
