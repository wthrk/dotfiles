...
- 補助記録: 未解決 10 は `review.md` / `confirmation.md` / 本ファイルにも記録されているが、これらの同内容同期そのものは gate ではない
...
- **レビュー/状態記録の扱い**
  ...
  - 新たな実コード差分と必要な確認結果が揃うまで、固定実装単位トラッカーの `確認` / `レビュー` / `必要時の後続対応` は `未着手` のまま維持する。
...
- 全 usecase は port trait 由来の `mockall` mock を使って app/usecase test を持つこと。独自 test double の追加は禁止する。
