...
- `direnv exec . cargo check -p dotfiles-cli` で成功を確認する
- トラッカーのそのエントリを `完了` に更新する
- **コミットを必ず実行する**。`git add` で変更ファイルをステージングし、`git commit -m "refactor(secrets): #12 ステップN でVX を解消"` の形式でコミットする。コミットが完了するまでセッションを終了しない。
- 次のステップには進まず停止する
