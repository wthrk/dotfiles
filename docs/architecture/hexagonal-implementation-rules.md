...
- comment は恒久的な設計意図、不変条件、制約、自明でない運用上の文脈を記し、単なるコード言い換え、個人メモ、曖昧な TODO/FIXME を禁止する。禁止対象は冗長な comment であり、上記必須対象を省く根拠にしてはならない。
- 処理の途中に置いて次の行を言い換えるだけの文中コメントを書かない。調べた過程、実測した出力、内部機構の解説を並べる備忘録コメントも書かない。経緯は PR body に置く。文中に残してよいのは、その行の読み手が保守判断に使う理由だけである。
