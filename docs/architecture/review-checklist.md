...
- adapter 配下 `stub` module が存在する場合、`secrets-internal-test-stub` feature 専用・production build 非混入・runtime real/stub 分岐なし・production command path 不変・port 契約維持・integration test が binary 実行のみで stub module を import しない、の条件を満たすか。満たす場合は存在だけで混入判定しない。
...
- fixture/state helper は tests 側責務、backend stub 実装は adapter 側責務として分離されているか。境界が混ざっていれば不合格とする。
...
- **許可**: adapter 配下 `stub` module にある backend stub 実装。許可条件は `secrets-internal-test-stub` feature 専用、production build 非混入、runtime real/stub 分岐なし、production command path 不変、同一 port 契約維持、integration test が feature 有効ビルドの `dotfiles` binary を実行し adapter stub module を import しない、fixture/state helper は tests 側責務、domain/business logic を stub 側へ移さない、をすべて満たすこと。
