...
### 証跡同期コミットの自己 hash 非要求

review artifact / ledger / work item を同期する文書-only commit は、自分自身の commit hash を本文へ埋め込む必要はない。Git commit hash は commit 本文を含むため、自己 hash 固定を要求すると固定点にならず、証跡同期 commit のたびに記録がずれ続ける。

レビュー開始条件と集約条件で要求するのは、`実装/テスト差分の保存コミット終端` と、証跡同期が文書-only HEAD として識別できることまでである。証跡同期 commit 自身は `この証跡同期コミット`、`文書-only HEAD`、または `git log の HEAD で確認` として扱える。運用整合レビューおよび参照整合レビューは、証跡同期 commit 自身の hash が本文に固定されていないことを単独の `要修正` / `不合格` 根拠にしてはならない。
