...
作業定義文書の `規約違反の解消対象`（secret-recovery work-item では `境界維持の観点`）・`構造完了条件`・`完了条件` を現行コードに対して直接照合する。各項目について現行コードを開いて未解消が残っていないかを確認する。サマリーや実装担当の報告で代替してはならない。
...
review artifact / confirmation / 台帳などの補助記録は、対象差分・レビュー結果・必要な実検証を補助的に追跡するための記録であり、記録以上の意味を持たない。これらは実装品質・レビュー品質・検証正確性の代替にも、追加の review gate / commit gate にもしてはならない。

確認記録の完全コマンド化、全 hit 行の網羅列挙、no-hit 出力の完全保存、追試可能性の最大化そのものを review gate / commit gate にしてはならない。文書是正や PR review 対応で必要な最小記録は、「何の変更を確認したか」「どの確認を実行したか」「結果が成功/該当/非該当のどれか」で足りる。対象差分本文や必須レビューで直接確認できる内容について、confirmation の `...`・要約記載・行番号列挙不足・file-set/current-cycle 文言の非完全同期といった補助記録の不備だけを理由に `要修正` / `不合格` / commit gate 未充足としてはならない。

一方で、実体側の要件が未充足な場合は指摘できる。具体的には、対象差分そのものが特定不能、必須レビュー結果が存在しない、必要な実検証の実施有無が判断不能、領域固有の実質条件が未充足、のいずれかに該当する場合である。
...
- `主成果物: 実コード差分` の作業項目で `集約後レビュー判定: 合格` とする場合、合否判定を要約だけで決めてはならない。作業定義文書の `規約違反の解消対象`（secret-recovery work-item では `境界維持の観点`）と `構造完了条件` に対し、現行コード追跡で未解消項目が残っていないことを確認できる場合に限り `合格` とできる。
