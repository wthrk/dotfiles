...
この機能は file 名をそれらしく分けるだけでは不十分で、application が concrete helper を直接参照しない構成を必須にする。Rust module は次の責務で固定する。
...
- `ports/device.rs`
  - `SecretDeviceFactory`
  - `SecretDevicePort`
  - `SpareDeviceSelectionPort`
- `ports/io.rs`
  - `SecretInputPort`
  - `SecretOutputPort`
  - `TerminalPromptPort`

port は用途別に分ける。device と terminal I/O を 1 trait に混在させない。application は command ごとに必要な port だけを generic parameter または境界 struct として受け取る。
...
- `adapters/yubikey/`
  - 実機 YubiKey adapter
  - test stub adapter
  - reader 選択と spare 差し替え待機
- `adapters/terminal/`
  - hidden prompt
  - visible prompt
  - `--stdin` / `--stdin-json`
  - stdout TTY 拒否
  - interrupt-aware prompt

adapter は free function 集ではなく、port trait を実装する明示的な型として持つ。application から `adapters::*` の関数を直接 import する構成は禁止する。

### Application DTO

- enrollment 用の 3 secret set
- rotation の部分成功 summary
- command ごとの request/response 補助型

これらは `application/` 配下に置く。port file に application 専用 DTO を置かない。

### Blob / 保存仕様

`SecretBlob` の encode/decode、blob 暗号化、blob 復号、AEAD additional data の組み立ては保存仕様の一部として扱う。top-level の `crypto.rs` や `input.rs` のような責務不明 file は作らず、domain 寄り module にまとめる。

### 禁止事項

- application が `adapters::input::*` のような concrete helper を直接呼ぶ
- adapter が application helper を import する
- port trait が `stdin_is_terminal`、`open_device`、`prompt_yes_no`、`read_secret` を全部まとめて持つ
- file の移動だけで trait 契約が変わっていない状態を「hexagonal 対応」と扱う
