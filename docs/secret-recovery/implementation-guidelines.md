...
- secret-recovery で役割起動が必要な全工程（固定実装単位 6 項目である `規約計画`、`実装計画`、`規約文書更新`、`確認`、`レビュー`、`必要時の後続対応` と、進捗更新工程 `進捗判定`）では、必要役割を起動できない場合に同一の役割別フォールバック記録 6 項目を必須とする。記録なしで停止または保留してはならない。
- 上記フォールバックでは、current orchestrator/current executor は対象役割の代替実行者になってはならない。代替実行者は当該役割に未割当の fresh agent/run を用い、no-reuse 規則を証跡で示す。
...
- secret-recovery の固定実装単位 6 項目（`規約計画`、`実装計画`、`規約文書更新`、`確認`、`レビュー`、`必要時の後続対応`）と進捗更新工程 `進捗判定` では、担当した各役割ごとに `agent 識別子` と `run 識別子` を同一変更セットの証跡へ記録する。
