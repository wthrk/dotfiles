...
- `進捗判定担当`: [../tasks/secret-recovery/tasks.md](../tasks/secret-recovery/tasks.md#新規マシン秘密情報復旧基盤タスク) と [../tasks/secret-recovery/issue-11-progress.md](../tasks/secret-recovery/issue-11-progress.md#11-系粗粒度進捗) の状態更新を行う。
...
- `レビュー` で必要役割を起動できない場合は、役割別フォールバックを宣言し、`対象役割`、`起動失敗理由`、`起動失敗証跡`、`代替実行者`、`代替実行者 agent/run 識別子`、`no-reuse 規則充足根拠` を同一変更セットへ記録したうえでレビューを継続する。記録なしで停止または保留してはならない。
- `進捗判定` は進捗判定担当が行う。オーケストレーターは進捗判定担当を兼務する明示的フォールバック記録がない限り、`確認` / `レビュー` / `実装状態` の前進記録を直接記入してはならない。証跡が欠ける前進更新は無効とする。
- `#11` 系の粗粒度進捗が欠落している場合は、実装開始前または並行で履歴復元担当を入れる。

## 役割別 agent/run 記録義務（fresh-subagent / no-reuse 監査）

- secret-recovery の `規約計画`、`実装計画`、`規約文書更新`、`確認`、`レビュー`、`進捗判定`、`必要時の後続対応` では、担当した各役割ごとに `agent 識別子` と `run 識別子` を同一変更セットの証跡へ記録する。
- `レビュー` と `進捗判定` は、役割別判定欄に加えて `agent/run` 識別子を必須記録とし、未記録の判定は無効とする。
- 役割起動に失敗した場合は、対象役割の `agent/run` 欄を空欄のままにせず、`起動失敗理由`、`起動失敗証跡`、`代替実行者`、`代替実行者 agent/run 識別子`、`no-reuse 規則充足根拠` を同一記録内で結びつける。
...
- [../tasks/secret-recovery/tasks.md](../tasks/secret-recovery/tasks.md#新規マシン秘密情報復旧基盤タスク) は進捗台帳であり、仕事の内容定義は [../tasks/secret-recovery/work-items/README.md](../tasks/secret-recovery/work-items/README.md#secret-recovery-work-items) を正本とする。
