...
2. BWS から取得した `gpg-secret-key-backup` をメモリ上の `gpgme::Data` として扱う。
3. `Context::import` で鍵リングへ投入する。
4. import result から対象 primary fingerprint を特定し、同一 fingerprint の key を再取得して subkey 検証へ渡す。
...
- `SSH_AUTH_SOCK="$(gpgconf --list-dirs agent-ssh-socket)"`（取得失敗時のみ既存値へフォールバック）
...
- `gpg-secret-key-backup` を取得し、GPG secret key を import する。
- encryption / authentication / signing subkey の存在を検証する。
