...
use crate::secrets::{
    domain::{self, PivObjectId, SecretBlob, SecretManifest, SecretName, CONTENT_KEY_LEN, NONCE_LEN},
    ports::SecretDevice,
    support::{
...
impl TestDevice {
    ...
    fn read_seed_secret<'session>(
        ...
        self.decrypt_seed_secret_protected(&blob, session)
    }

    /// 保存用 blob を検証し、secret 本文を保護済み値へ復号する。
    fn decrypt_seed_secret_protected<'session>(
        &mut self,
        blob: &SecretBlob,
        session: &'session SecretSession,
    ) -> Result<ProtectedSecret<'session>> {
    ...
    fn emit_write_event(&mut self, object_id: PivObjectId) -> Result<()> {
