...
//!
//! `bincode` 2.0.1、`rand_core` 0.6.4、`serde` 1.0.228、`zeroize` 1.8.2 の API / error policy は
...
impl SealedBlob {
    fn encode(&self) -> Result<Vec<u8>> {
        let payload = bincode::serde::encode_to_vec(self, config::standard())
            .context("failed to encode sealed blob")?;
    ...
    fn decode(input: &[u8]) -> Result<Self> {
        ...
        let (blob, read) =
            bincode::serde::decode_from_slice::<Self, _>(payload, config::standard())
                .context("failed to decode sealed blob")?;
        if read != payload.len() {
            return invalid_blob();
        }
...
pub(crate) fn seal_with_key_wrap(
    ...
    secret_random::fill_os_random(&mut nonce, "sealed-blob nonce")?;
...
mod tests {
    ...
    fn sealed_blob_round_trips_without_aliasing_plaintext() -> Result<()> {
        ...
        opened.with_secret(|bytes| assert_eq!(bytes, b"secret-value"));
        Ok(())
    }
