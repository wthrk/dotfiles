...
impl ProtectedSecret {
    ...
    /// PIV PIN protocol が許可する ASCII alphanumeric byte 列かを返す。
    ///
    /// plaintext は返さず、PIV input backend が mutation 前 validation に使うだけの境界である。
    pub(crate) fn is_ascii_alphanumeric(&self) -> bool {
        self.with_secret(|bytes| bytes.iter().all(u8::is_ascii_alphanumeric))
    }
...
mod tests {
    ...
    fn try_clone_returns_independent_locked_copy() -> Result<()> {
        ...
        cloned.with_secret(|bytes| assert_eq!(bytes, b"abc"));
        original.with_secret(|bytes| assert_eq!(bytes, b"xyz"));
        Ok(())
    }
    ...
    fn try_clone_copy_survives_source_drop() -> Result<()> {
        ...
        cloned.with_secret(|bytes| assert_eq!(bytes, b"persist"));
        Ok(())
    }
}
