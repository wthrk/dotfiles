...
mod tests {
    ...
    #[cfg(not(feature = "secrets-internal-test-stub"))]
    use crate::{Result, support::protection::SecretSession};
    ...
    #[cfg(not(feature = "secrets-internal-test-stub"))]
    fn assert_hidden_too_long(result: Result<crate::support::protection::ProtectedSecret>) {
    ...
    fn pty_hidden_prompt_accepts_pasted_bracketed_input() -> Result<()> {
        ...
        command.args([
            "--exact",
            "support::process_io::tests::pty_hidden_child_reads_pasted_input",
            "--nocapture",
        ]);
    ...
    fn pty_hidden_prompt_rejects_over_limit_pasted_bracketed_input() -> Result<()> {
        ...
        command.args([
            "--exact",
            "support::process_io::tests::pty_hidden_child_rejects_over_limit_pasted_input",
            "--nocapture",
        ]);
        command.env(PTY_CHILD_ENV, "1");
        let mut child = pair.slave.spawn_command(command)?;
    ...
    fn pty_write_secret_stdout_rejects_terminal() -> Result<()> {
        ...
        command.args([
            "--exact",
            "support::process_io::tests::pty_write_secret_child_rejects_terminal",
            "--nocapture",
        ]);
        command.env(PTY_CHILD_ENV, "1");
        let mut child = pair.slave.spawn_command(command)?;
