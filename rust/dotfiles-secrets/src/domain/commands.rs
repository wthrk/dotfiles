...
impl EnrollSpareCommand {
    ...
    /// source が解決した primary と spare が別 device であることを確認する。
    ///
    /// primary source は technical discovery の結果だけを返す。role の同一性判定は external
    /// backend へ委ねず、application がこの domain invariant を適用する。
