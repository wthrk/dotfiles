...
/// `sealed_with_generated_key` 観測で封緘鍵を照合し、PIN 必須時は enroll-spare が PIN 取得つき init で
/// 鍵生成まで進み、低水準 `yubikey setup` の `setup_fails_closed_when_key_generation_requires_pin` が
/// PIN 未検証停止を照合する）に置く。
#[cfg(test)]
mod tests {
