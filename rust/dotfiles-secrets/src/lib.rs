...
/// 他の state transition の根拠にせず、高水準 `enroll-primary` / `enroll-spare` を使う。
...
pub fn is_secret_storage_status_invalid(error: &anyhow::Error) -> bool {
    error
        .chain()
        .any(features::yubikey_lifecycle::ports::public::is_status_invalid_cause)
}
...
pub fn is_secret_storage_uninitialized(error: &anyhow::Error) -> bool {
    error
        .chain()
        .any(features::yubikey_lifecycle::ports::public::is_uninitialized_cause)
}
...
enum YubikeyCommand {
    ...
    /// 初期化済み BWS token storage の観測・必要時保存・復号検証を単一 PIV session で行う。
    ///
...
enum GpgBackupCommand {
    /// 外部 state mutation 前に source secret key の復旧可能性を検査する。
    Validate(GpgBackupValidateOptions),
    Register(GpgBackupRegisterOptions),
    AddSpare(GpgBackupAddSpareOptions),
}

#[derive(Args)]
/// passphrase-free transferable secret key と required E/A/S capabilities を非変更で検査する option。
struct GpgBackupValidateOptions {
    #[arg(long)]
...
pub async fn run(options: SecretsOptions) -> Result<()> {
    composition::bootstrap::start(options).await
}
...
pub fn run_gpg(options: GpgOptions) -> Result<()> {
    composition::bootstrap::start_gpg(options)
}
