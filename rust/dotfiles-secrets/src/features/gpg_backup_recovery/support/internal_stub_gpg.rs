    ...
    features::{
        gpg_backup_recovery::domain::{
            ...
            gpg_restore::{
                ...
                ResolvedSubkey, SshAgentReadiness, SubkeyCapability,
            },
        },
...
struct Spec {
    ...
    #[serde(default)]
    backup_certificate_revocation: RevocationProfile,
    #[serde(default)]
    backup_subkey_revocation: RevocationProfile,
}
fn truth() -> bool {
    true
}
...
fn with_store<T>(f: impl FnOnce(&mut Store) -> Result<T>) -> Result<T> {
    ...
    if guard.is_none() {
        ...
        *guard = Some(Store {
            ...
            backup_certificate_revocation: spec.backup_certificate_revocation,
            backup_subkey_revocation: spec.backup_subkey_revocation,
        });
    }
    let store = guard
        .as_mut()
        .ok_or_else(|| anyhow::anyhow!("GPG internal stub datastore was not initialized"))?;
    let result = f(store)?;
    println!(
...
pub(crate) fn register_keygrip(keygrip: &str) -> Result<bool> {
    with_store(|store| {
...
pub(crate) fn unregister_keygrip(keygrip: &str) -> Result<()> {
    with_store(|store| {
        store.registered.retain(|value| value != keygrip);
        Ok(())
    })
}
...
pub(crate) fn unregister_authentication_subkey(keygrip: &Keygrip) -> Result<()> {
    unregister_keygrip(keygrip.as_str())
}
