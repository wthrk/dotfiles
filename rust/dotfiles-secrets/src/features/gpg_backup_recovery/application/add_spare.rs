...
pub(crate) async fn run_add_gpg_backup_spare<B>(
    ...
    confirmation: &dyn BackupUpdateConfirmationPort,
) -> Result<()>
where
...
#[cfg(test)]
mod tests {
    //! spare 追加の順序（device 解決→YubiKey token 取得→envelope 取得→確認→unwrap→再 wrap→
    //! guard 更新）を mockall + Sequence で検証する単体テスト。
    //!
    //! YubiKey storage / recipient / bws / confirmation backend を port mock で差し替え、BWS 更新に使う
    //! access token を unwrap 機の `bitwarden-client-secret` から取得すること、確認を通過した場合だけ
    ...
    mod ports {
        ...
        pub(crate) use crate::features::cli_interaction::ports::public::MockBackupUpdateConfirmationPort;
    ...
    /// BWS access token を unwrap 機の YubiKey storage から読む port mock を共通設定する。
    ...
    fn connected_unwrap() -> crate::Result<ConnectedYubiKey> {
    ...
    fn spare_recipient() -> crate::Result<EnvelopeRecipient> {
        let connected = ConnectedYubiKey::new(
            "2002",
            "fedcba9876543210fedcba9876543210fedcba9876543210fedcba9876543210",
        )?;
        EnvelopeRecipient::new(&connected, b"wrapped-spare".to_vec())
    }
    ...
    /// 固定する。
    #[tokio::test]
    async fn add_spare_stops_when_guard_mismatch_blocks_update() -> crate::Result<()> {
        let mut yk = yubikey_access_mocks();
        yk.device
            .expect_resolve_device_serial()
            .times(1)
    ...
    /// 確認が拒否された場合、DEK unwrap・spare wrap・guard 更新のいずれにも進ませず、
    /// DEK 復号を発生させないことを検証する。
    #[tokio::test]
    async fn add_spare_rejection_skips_unwrap_and_update() -> crate::Result<()> {
        let mut yk = yubikey_access_mocks();
        yk.device
            .expect_resolve_device_serial()
            .times(1)
