...
mod tests {
    ...
    mod ports {
        ...
        pub(crate) use crate::features::cli_interaction::ports::public::MockClockPort;
        pub(crate) use crate::features::gpg_backup_recovery::ports::public::{
            MockBackupCipherPort, MockGpgKeyringPort,
        };
    ...
    fn backup_facts() -> crate::Result<OpenPgpBackupFacts> {
        let primary_fingerprint = PrimaryFingerprint::parse(PRIMARY_FP)?;
    ...
    struct YubiKeyAccessMocks {
        device: crate::features::yubikey_lifecycle::ports::public::MockDeviceSerialPort,
        storage: crate::features::yubikey_lifecycle::ports::public::MockSecretStoragePort,
    }
    ...
    fn ciphertext() -> crate::Result<EnvelopeCiphertext> {
        EnvelopeCiphertext::new(vec![0u8; 12], b"encrypted".to_vec(), vec![0u8; 16])
    }

    fn recipient_entry() -> crate::Result<EnvelopeRecipient> {
    ...
    async fn register_primary_creates_envelope_when_absent() -> crate::Result<()> {
        ...
        bws.expect_create_gpg_backup_envelope()
            ...
            .returning(|_, _, _, _| Ok(domain::bws::BwsSecretId::new("new-id")));
    ...
    #[tokio::test]
    async fn register_primary_stops_when_duplicate_secrets_exist() -> crate::Result<()> {
        let mut yk = yubikey_access_mocks();
        ...
        bws.expect_list_bws_secrets().returning(|_, _| {
            Ok(vec![
                domain::bws::BwsLookupCandidate {
                    id: domain::bws::BwsSecretId::new("dup-1"),
                    name: "gpg-secret-key-backup".to_owned(),
                },
                domain::bws::BwsLookupCandidate {
                    id: domain::bws::BwsSecretId::new("dup-2"),
                    name: "gpg-secret-key-backup".to_owned(),
                },
            ])
        });
    ...
    /// packet-level protection validation が失敗した backup は、cipher/recipient/BWS mutation に進ませない。
    #[tokio::test]
    async fn register_primary_stops_before_bws_create_when_backup_packet_is_protected()
    -> crate::Result<()> {
        let mut yk = yubikey_access_mocks();
        let mut bws = ports::MockBwsClientPort::new();
        bws.expect_list_bws_projects().returning(|_| {
        ...
        keyring
            .expect_export_secret_key()
            .returning(|_| material(b"protected-packet-fixture"));
        keyring
            .expect_parse_backup_primary_fingerprint()
            .returning(|_| {
                Err(anyhow::anyhow!(
        ...
        let error = run_register_gpg_backup_primary(
            RegisterGpgBackupCommand {
                primary_fingerprint: Some(PrimaryFingerprint::parse(PRIMARY_FP)?),
                serial: Some(2001),
            },
        ...
        .await
        .expect_err("protected backup must stop before BWS create");
