...
pub trait SshAgentPort {
    /// authentication subkey の keygrip を gpg-agent の SSH key list へ登録する。
    ///
    /// 戻り値はこの invocation が entry を新規作成したかを表す。後続の SSH readiness 観測が失敗した場合、
    /// caller は `true` の entry だけを削除して、実行前から存在した identity を変更してはならない。
    fn register_authentication_subkey(&mut self, keygrip: &Keygrip) -> Result<bool>;

    /// この invocation が登録した authentication subkey の keygrip を SSH key list から除去する。
    ///
    /// `register_authentication_subkey` が `true` を返した場合にだけ caller が呼ぶ rollback capability である。
