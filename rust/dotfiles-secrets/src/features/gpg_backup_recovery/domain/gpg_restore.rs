...
use crate::{Result, features::gpg_backup_recovery::domain::gpg_backup::PrimaryFingerprint};

/// Sequoia が certificate/subkey について返す revocation state の閉じた写像。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OpenPgpRevocation {
    Revoked,
    CouldBe,
    NotAsFarAsWeKnow,
}

/// 一つの transferable-secret-key subkey から得た外部 SDK facts。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct OpenPgpSubkeyFacts {
    pub supported: bool,
    pub alive: bool,
    pub revocation: OpenPgpRevocation,
    pub secret: bool,
    pub signing: bool,
...
/// OpenPGP backup の SDK facts を domain recovery policy へ渡す境界値。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct OpenPgpBackupFacts {
    pub primary_fingerprint: PrimaryFingerprint,
    pub certificate_revocation: OpenPgpRevocation,
    pub subkeys: Vec<OpenPgpSubkeyFacts>,
}

impl OpenPgpBackupFacts {
    /// certificate が non-revoked で、secret signing + authentication +
    /// (storage OR transport encryption) subkey を持つことを要求する。
    pub fn ensure_recovery_capabilities(self) -> Result<PrimaryFingerprint> {
        if self.certificate_revocation != OpenPgpRevocation::NotAsFarAsWeKnow {
            anyhow::bail!("OpenPGP backup certificate is revoked or has an unresolved revocation");
        }
        let usable = |subkey: &&OpenPgpSubkeyFacts| {
            subkey.supported
                && subkey.alive
                && subkey.revocation == OpenPgpRevocation::NotAsFarAsWeKnow
                && subkey.secret
        };
        let signing = self.subkeys.iter().filter(usable).any(|key| key.signing);
        let authentication = self
            .subkeys
            .iter()
            .filter(usable)
            .any(|key| key.authentication);
        let encryption = self
            .subkeys
            .iter()
            .filter(usable)
            .any(|key| key.storage_encryption || key.transport_encryption);
        if !signing || !authentication || !encryption {
            anyhow::bail!(
                "OpenPGP backup lacks a supported, alive, non-revoked secret signing, authentication, or encryption subkey"
            );
        }
        Ok(self.primary_fingerprint)
    }
}
...
mod tests {
    ...
    fn backup_facts(
        certificate_revocation: OpenPgpRevocation,
        encryption: (bool, bool),
    ) -> crate::Result<OpenPgpBackupFacts> {
        let usable = |signing, authentication, storage_encryption, transport_encryption| {
            OpenPgpSubkeyFacts {
                supported: true,
                alive: true,
        ...
        Ok(OpenPgpBackupFacts {
            ...
            subkeys: vec![
                usable(true, false, false, false),
                usable(false, true, false, false),
                usable(false, false, encryption.0, encryption.1),
            ],
        })
    }

    #[test]
    fn recovery_backup_accepts_storage_or_transport_encryption() -> crate::Result<()> {
        assert!(
            backup_facts(OpenPgpRevocation::NotAsFarAsWeKnow, (true, false))?
                .ensure_recovery_capabilities()
                .is_ok()
        );
        assert!(
            backup_facts(OpenPgpRevocation::NotAsFarAsWeKnow, (false, true))?
                .ensure_recovery_capabilities()
                .is_ok()
        );
        Ok(())
    ...
    #[test]
    fn recovery_backup_rejects_revoked_or_ambiguous_certificate() -> crate::Result<()> {
        assert!(
            backup_facts(OpenPgpRevocation::Revoked, (true, false))?
                .ensure_recovery_capabilities()
                .is_err()
        );
        assert!(
            backup_facts(OpenPgpRevocation::CouldBe, (true, false))?
                .ensure_recovery_capabilities()
                .is_err()
        );
        Ok(())
    ...
    #[test]
    fn recovery_backup_rejects_unusable_required_subkey() -> crate::Result<()> {
        let mut facts = backup_facts(OpenPgpRevocation::NotAsFarAsWeKnow, (true, false))?;
        facts.subkeys[1].alive = false;
        assert!(facts.ensure_recovery_capabilities().is_err());
        Ok(())
    }
    ...
    #[test]
    fn keygrip_normalizes_and_validates() -> crate::Result<()> {
        let raw = "aabb:ccdd:eeff:0011:2233:4455:6677:8899:aabb:ccdd";
        let keygrip = Keygrip::parse(raw)?;
    ...
    #[test]
    fn openssh_public_key_accepts_valid_ed25519_line() -> crate::Result<()> {
        let line = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAITESTBODY comment";
        let key = OpenSshPublicKey::parse(line)?;
    ...
    #[test]
    fn matches_agent_key_blob_by_decoded_body() -> crate::Result<()> {
        ...
        let key = OpenSshPublicKey::parse("ssh-ed25519 YmxvYg== cardno:0006")?;
