...
/// presentation が構築済みの非秘匿な一行を process stdout へ書く。
...
use crate::foundation::protection::{ProtectedSecret, SecretSession, buffer::ProtectedInputBuffer};
...
/// raw mode を通常終了と unwind の両方で解除する guard。
///
/// 通常終了では [`RawModeGuard::finish`] が cleanup result を返す。panic/unwind では Drop が
/// best-effort 解除し、terminal を raw mode のまま残さない。
struct RawModeGuard {
    active: bool,
}
...
impl RawModeGuard {
    ...
    fn finish<T>(mut self, main: Result<T>) -> Result<T> {
        let cleanup = disable_raw_mode().map_err(anyhow::Error::new);
        self.active = false;
        finish_raw_mode(main, cleanup)
    }
}
...
/// raw-mode 本処理と cleanup の両結果を、主失敗の source を失わず一つの結果へ畳み込む。
///
/// 本処理成功後の cleanup failure は failure とする。本処理と cleanup が両方失敗した場合も、raw
/// terminal detail は outer Display に含めず source chain にだけ保持する。
fn finish_raw_mode<T>(main: Result<T>, cleanup: Result<()>) -> Result<T> {
    match (main, cleanup) {
        (Ok(value), Ok(())) => Ok(value),
        (Ok(_), Err(cleanup)) => Err(cleanup.context("failed to disable terminal raw mode")),
        (Err(main), Ok(())) => Err(main),
        (Err(main), Err(cleanup)) => Err(anyhow::Error::new(RawModeCleanupFailure {
            main,
            _cleanup: cleanup,
        })),
    }
...
#[derive(Debug)]
struct RawModeCleanupFailure {
    main: anyhow::Error,
    _cleanup: anyhow::Error,
}

impl std::fmt::Display for RawModeCleanupFailure {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("terminal raw-mode cleanup failed")
    }
}

impl std::error::Error for RawModeCleanupFailure {
...
#[cfg(test)]
mod raw_mode_cleanup_tests {
    //! hidden input の本処理と raw-mode cleanup の fault matrix を検証する。

    use super::finish_raw_mode;

    #[test]
    fn successful_input_with_cleanup_failure_returns_cleanup_source() -> crate::Result<()> {
        let error = finish_raw_mode(Ok::<_, anyhow::Error>(()), Err(anyhow::anyhow!("cleanup")))
            .err()
            .ok_or_else(|| anyhow::anyhow!("cleanup failure must fail the operation"))?;
        assert_eq!(error.to_string(), "failed to disable terminal raw mode");
        assert!(error.chain().any(|source| source.to_string() == "cleanup"));
        Ok(())
    }

    #[test]
    fn input_and_cleanup_failure_preserve_the_input_source() -> crate::Result<()> {
        let error = finish_raw_mode::<()>(
            Err(anyhow::anyhow!("input-source")),
            Err(anyhow::anyhow!("cleanup-source")),
        )
        .err()
        .ok_or_else(|| anyhow::anyhow!("combined failure must fail the operation"))?;
        assert_eq!(error.to_string(), "terminal raw-mode cleanup failed");
        assert!(!error.to_string().contains("cleanup-source"));
        assert!(
            error
                .chain()
                .any(|source| source.to_string() == "input-source")
        );
        Ok(())
    }
}
...
mod tests {
    ...
    fn controlling_tty_pin_bytes_reach_verify_unmodified_except_line_terminator() -> Result<()> {
        ...
        pin.with_secret(|bytes| observed.replace(bytes.to_vec()));
    ...
    fn delayed_crlf_is_not_interpreted_as_the_next_hidden_prompt_input() -> Result<()> {
        ...
        let pin = read_hidden_secret_input_from(&mut tty, 8, "too large")?;
...
mod plain_line_tests {
    ...
    #[test]
    fn reads_line_terminated_by_lf() -> crate::Result<()> {
        let mut reader: &[u8] = b"https://example.test/repo.git\nnext";
        let line = read_plain_line_from(&mut reader, 1024, TOO_LONG)?;
    ...
    #[test]
    fn reads_line_terminated_by_lone_cr() -> crate::Result<()> {
    ...
    #[test]
    fn crlf_is_consumed_as_single_newline_without_residual_lf() -> crate::Result<()> {
    ...
    #[test]
    fn line_without_terminator_returns_until_eof() -> crate::Result<()> {
        let mut reader: &[u8] = b"value";
        let line = read_plain_line_from(&mut reader, 1024, TOO_LONG)?;
        assert_eq!(line, "value");
        Ok(())
