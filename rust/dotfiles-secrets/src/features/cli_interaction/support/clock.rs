...
/// process の wall-clock を提供する command-neutral な technical receiver。
///
/// presentation は時刻 source を所有しない。composition がこの receiver を生成し、adapter が
/// `ClockPort` へ単一 forwarding することで、application は時刻 capability だけを借用する。
