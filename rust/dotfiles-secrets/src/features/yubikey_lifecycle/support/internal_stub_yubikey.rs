...
use super::yubikey_backend::{DeviceCandidate, SecretDeviceIo, SelectedSecretDevice};
...
struct YubiKeyDeviceSpec {
    ...
    /// `change_pin` API error 後に変更後 VERIFY/auth/storage mutation が起きないことを確認する
    /// feature 専用 fixture。production backend の状態遷移には存在しない。
    #[serde(default)]
    pin_change_fails: bool,
    /// key が存在しても certificate inspection error を PIN 変更前に伝播することを検証する
    /// feature 専用 fixture。production backend の状態遷移には存在しない。
    #[serde(default)]
    reserved_slot_certificate_read_fails: bool,
}
...
enum YubiKeyDeviceFixture {
    ...
    CorruptManifest,
    VersionOneManifest,
    ZeroLengthManifestWithReservedKey,
    ManifestWithoutReservedKey,
    ManifestWithMismatchedReservedKey,
    ManifestlessReservedKey,
...
struct StubDeviceDatastore {
    ...
    /// PIV slot 82 の private-key operation（storage unwrap / envelope DEK unwrap）を、値を
    /// 含めずに観測する feature-only counter。
    #[serde(default)]
    private_key_operation_count: u32,
    #[serde(default)]
    management_authentication_count: u32,
    key_metadata_requires_management_auth: bool,
    #[serde(default)]
...
struct YubiKeyObservation {
    /// secret input の試行回数だけを公開する feature-only observation。
    /// value / length / prompt は含めない。
    secret_input_attempt_count: u32,
...
struct StubDeviceObservation {
    ...
    reserved_slot_certificate_exists: bool,
    slot_public_key_spki: Option<Vec<u8>>,
    object_lengths: BTreeMap<String, usize>,
    connection_count: u32,
    physical_verify_count: u32,
    /// PIN-free recovery が physical touch を伴う private operation へ到達したことだけを示す。
    /// PIN、DEK、token、wrapped value は observation に含めない。
    private_key_operation_count: u32,
    /// PIN-free recovery path が management-key authentication を開始しないことを secret 値なしで観測する。
    management_authentication_count: u32,
    management_pin_matched: bool,
...
impl SecretDeviceIo for TestStubSecretDevice {
    ...
    fn reserved_slot_certificate_exists(&mut self) -> Result<bool> {
        with_datastore(|store| {
            let device = device_store(store, self.serial)?;
            if device.reserved_slot_certificate_read_fails {
                anyhow::bail!("stub reserved slot certificate read failed")
            }
    ...
    fn change_management_pin(
        ...
        with_datastore_after_write(|store| {
            let device = device_store_mut(store, self.serial)?;
            if device.pin_change_fails {
                anyhow::bail!("stub YubiKey PIN change failed")
            }
            if let Some(expected_pin) = device.expected_management_pin.as_deref() {
                let expected = ProtectedSecret::from_test_bytes(expected_pin.as_bytes())?;
                if current_pin != &expected {
                    anyhow::bail!("stub current PIV PIN does not match the fixture expectation")
                }
            }
            let new_pin = new_pin.to_test_bytes();
            let new_pin = String::from_utf8(new_pin)
                .map_err(|_| anyhow::anyhow!("stub PIV PIN is not UTF-8"))?;
            device.expected_management_pin = Some(new_pin);
            Ok(())
        })
    }

    fn authenticate_protected_management_key(&mut self) -> Result<()> {
        let state = with_datastore_after_write(|store| {
            let device = device_store_mut(store, self.serial)?;
            device.management_authentication_count += 1;
            Ok(device.management_state)
        })?;
        match state {
    ...
    fn generate_key(&mut self) -> Result<Vec<u8>> {
        with_datastore_after_write(|store| {
            ...
            let public_key_spki = SecretManifest::fixture_v2()
                ...
                .ok_or_else(|| anyhow::anyhow!("fixture manifest does not include SPKI"))?;
    ...
    fn open_from_storage(
        ...
        let value = with_datastore_after_write(|store| {
            let key = secret_key(storage.secret_id);
            let device = device_store_mut(store, self.serial)?;
            if device.corrupt.iter().any(|stored| stored == key) {
                anyhow::bail!("corrupt {key}");
            }
            // slot 82 の private unwrap は physical touch を要求し得るが、PIV PIN VERIFY や
            // management-key authentication ではない。feature-only observation は回数だけを保持する。
            device.private_key_operation_count += 1;
    ...
    fn unwrap_dek(&mut self, wrapped_dek: &[u8]) -> Result<ProtectedSecret> {
        with_datastore_after_write(|store| {
            device_store_mut(store, self.serial)?.private_key_operation_count += 1;
            Ok(())
        })?;
            ...
            .map(|(serial, device)| {
                ...
                Ok(DeviceCandidate {
                    ...
                    profile: domain::piv::PivDeviceProfile {
                        ...
                        fips_series: device.fips_series,
                    },
                })
            })
            .collect()
...
fn datastore_from_spec(spec: YubiKeyStubSpec) -> Result<YubiKeyDatastore> {
    let devices = spec
        .yubikeys
        .into_iter()
        .map(|device| {
        ...
        .map(|(serial, device)| {
            (
                ...
                PersistentStubDeviceDatastore {
                    ...
                    private_key_operation_count: device.private_key_operation_count,
                    management_authentication_count: device.management_authentication_count,
                    ...
                    reserved_slot_certificate_read_fails: device
                        .reserved_slot_certificate_read_fails,
                    ...
                    pin_change_fails: device.pin_change_fails,
                },
            )
        })
        .collect();
        ...
        .map(|(serial, persisted)| {
            Ok((
                ...
                StubDeviceDatastore {
                    ...
                    fips_series: persisted.fips_series,
                    connection_count: persisted.connection_count,
                    physical_verify_count: persisted.physical_verify_count,
                    private_key_operation_count: persisted.private_key_operation_count,
                    management_authentication_count: persisted.management_authentication_count,
                    ...
                    reserved_slot_certificate_read_fails: persisted
                        .reserved_slot_certificate_read_fails,
                    ...
                    pin_change_fails: persisted.pin_change_fails,
                },
            ))
        })
        .collect::<Result<BTreeMap<_, _>>>()?;
...
fn device_datastore_from_spec(spec: YubiKeyDeviceSpec) -> Result<StubDeviceDatastore> {
    let mut device = match spec.fixture {
        ...
        YubiKeyDeviceFixture::VersionOneManifest => {
            let mut device = provisioned_device_datastore(default_secrets())?;
            ...
            device
                .objects
                .insert(object_key(MANIFEST_OBJECT_ID), manifest.encode()?);
            device
        }
        YubiKeyDeviceFixture::ZeroLengthManifestWithReservedKey => {
            let mut device = StubDeviceDatastore {
                key_exists: true,
            ...
            device
                .objects
                .insert(object_key(MANIFEST_OBJECT_ID), Vec::new());
            device
        }
        ...
        YubiKeyDeviceFixture::ManifestWithMismatchedReservedKey => {
            let mut device = provisioned_device_datastore(default_secrets())?;
            if let Some(spki) = device.slot_public_key_spki.as_mut() {
                if let Some(last) = spki.last_mut() {
                    *last ^= 1;
                }
            }
            device
        }
        YubiKeyDeviceFixture::ManifestlessReservedKey => StubDeviceDatastore {
            key_exists: true,
            slot_public_key_spki: SecretManifest::fixture_v2().slot_public_key_spki,
            ..StubDeviceDatastore::default()
        },
        ...
        YubiKeyDeviceFixture::StatusReadFailure => {
            let mut device = provisioned_device_datastore(default_secrets())?;
            device.status_read_failure = true;
            device
        }
    ...
    device.fips_series = spec.fips_series;
    ...
    device.pin_change_fails = spec.pin_change_fails;
    device.reserved_slot_certificate_read_fails = spec.reserved_slot_certificate_read_fails;
    Ok(device)
}

fn provisioned_device_datastore(secrets: BTreeMap<String, String>) -> Result<StubDeviceDatastore> {
    ...
    objects.insert(object_key(MANIFEST_OBJECT_ID), manifest.encode()?);
    Ok(StubDeviceDatastore {
        key_exists: true,
        fips_series: false,
        connection_count: 0,
        physical_verify_count: 0,
        private_key_operation_count: 0,
        management_authentication_count: 0,
        ...
        reserved_slot_certificate_read_fails: false,
        ...
        .map(|(serial, device)| {
            (
                ...
                StubDeviceObservation {
                    ...
                    reserved_slot_certificate_exists: device.reserved_slot_certificate_exists,
                    slot_public_key_spki: device.slot_public_key_spki.clone(),
                    object_lengths: device
                        .objects
                        .iter()
                        .map(|(object_id, value)| (object_id.clone(), value.len()))
                        .collect(),
                    connection_count: device.connection_count,
                    physical_verify_count: device.physical_verify_count,
                    private_key_operation_count: device.private_key_operation_count,
