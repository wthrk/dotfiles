...
use crate::features::yubikey_lifecycle::domain::storage::{
    SecretStorageReadInspection, SecretStorageReadIntent,
};
...
impl SecretManifest {
    ...
    pub fn validate_slot_public_key_spki(spki: &[u8]) -> Result<()> {
        let public_key = RsaPublicKey::from_public_key_der(spki)
            .context("YubiKey slot 82 public key is not DER SPKI")?;
...
mod tests {
    ...
    #[test]
    fn manifest_rejects_non_rsa2048_spki() -> crate::Result<()> {
        use rand_core::OsRng;
