...
- **Evidence-sync self hash**: When review artifacts distinguish an implementation/test terminal commit from a document-only evidence-sync commit, do not require the evidence-sync commit to embed its own hash in its body. Git commit hashes include the commit body, so self-hash fixation is impossible and must not be used as an operational gate. It is sufficient that the artifact identifies the implementation/test terminal commit and says that the current evidence-sync commit is checked via `git log` / current HEAD.
