---
name: test-review
description: テストレビュー担当として判定するときに使う。
---
...
このスキル有効時の現在アクターは **test reviewer**。
...
7. ユーザー指定の GitHub issue、PR、明示タスク、または委譲されたレビュー入力
8. 入力が要求する追加正本文書
...
- test double、fixture、inline unit test、internal backend stub、test-only observation boundary を判定する場合は、architecture の test 配置規則を適用する。
