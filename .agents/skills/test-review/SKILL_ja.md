...
- **internal backend stub の例外**: adapter 配下の `secrets-internal-test-stub` feature 専用 backend stub は、production build 非混入、runtime 分岐なしの compile-time selection、同一 production command path、同一 port 契約、domain/business logic 非移動、integration test が adapter stub module を import しないこと、fixture/state helper が `tests/` 側責務として残ることを満たす場合に限り許可する。欠けがあれば `Verdict: Fail`。
