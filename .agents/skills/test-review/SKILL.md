...
- **Adapter internal backend stub exception**: Do not treat an adapter `#[cfg(feature = "secrets-internal-test-stub")]` backend stub as test-double leakage when it satisfies the canonical internal-backend-stub conditions: production build non-inclusion, internal-test feature only, compile-time backend selection rather than runtime real/stub branching, unchanged production command path, preserved port trait contract, no domain/business logic moved into the stub, integration tests not importing adapter stub modules, and fixture/state helper responsibility remaining in `tests/`.
