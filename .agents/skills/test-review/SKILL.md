...
- **Test-double placement distinction**: Do not fail solely because `adapters/stub` exists. Pass only when it is `secrets-internal-test-stub` feature-only, excluded from production builds, keeps runtime switching out of scope (compile-time selection only), preserves the production command path/port contracts, keeps fixture helpers under `tests/`, and integration tests execute the feature-enabled `dotfiles` binary without importing adapter stub modules.
