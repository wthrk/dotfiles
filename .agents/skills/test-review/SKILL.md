...
- **Internal backend stub exception**: Adapter-local backend stubs gated only by `secrets-internal-test-stub` are allowed only when they satisfy the canonical internal backend stub criteria in `docs/architecture/hexagonal-implementation-rules.md` and the responsibility checks in `docs/architecture/review-checklist.md`. Do not duplicate those criteria here; read the canonical documents and return `Verdict: Fail` if the implementation does not satisfy them.
