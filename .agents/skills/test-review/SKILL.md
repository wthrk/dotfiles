...
- **Adapter internal backend stub exception**: Do not treat an adapter `#[cfg(feature = "secrets-internal-test-stub")]` backend stub as test-double leakage when it satisfies `docs/architecture/hexagonal-implementation-rules.md` の `internal backend stub の配置` 節の canonical conditions.
