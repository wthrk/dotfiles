...
- For adapter `#[cfg(feature = "secrets-internal-test-stub")]` backend stubs, apply the `internal backend stub` conditions in `docs/architecture/hexagonal-implementation-rules.md` before treating them as production-source test double mixing. If all conditions are met, do not fail solely because the stub exists under `adapters/`. This does not relax adapter visibility: unnecessary `pub(super)` helpers still fail, and fixture/state helper responsibilities must remain outside the adapter backend stub.
