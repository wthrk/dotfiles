...
- adapter 配下の `secrets-internal-test-stub` feature 専用 backend stub は、同一 production command path、同一 port 契約、compile-time selection、tests 側 fixture/state 分離が保たれている限り、その存在だけで全体非整合と判定しない。
