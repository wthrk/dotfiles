...
- For adapter-local `secrets-internal-test-stub` backend stubs, judge whole-module coherence by the canonical internal backend stub conditions. Do not treat the mere presence of a feature-gated adapter stub as whole-design incoherence when production command path, port contract, compile-time selection, and tests-side fixture/state separation remain intact.
