...
- Philosophical review obligation: For each layer encountered in the diff, ask the philosophical question for that layer before applying checklist items. The questions are defined in `docs/architecture/review-checklist.md` under "レビュー時の問い" for each layer section. A file that passes all checklist items but fails a philosophical question must receive `判定: 不合格`. Checklist compliance is necessary but not sufficient for a passing verdict.
- When reviewing `ports/` files or `ports.rs`: ask "is this a declaration of what the domain needs, or does it contain implementation details (SDK types, parsers, prompts, raw byte transfer types)?" A `struct` or `enum` in `ports/` that transfers raw bytes, encodes prompt text, or depends on a specific serialization format is a port violation regardless of whether its dependencies are limited to `domain`.
- When reviewing `application/` files: ask "does this code know only the sequence of operations, or does it know concrete implementations?" The presence of any concrete type from `adapters/` — even through a re-export or type alias — is a violation regardless of how many indirection layers exist.
