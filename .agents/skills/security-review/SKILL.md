...
description: Use this skill when a subagent is assigned as the security reviewer to review implementation diffs for secret exposure, unauthorized access paths, and privilege escalation risks.
---
...
Apply constraints defined in `docs/task-governance/security-obligations.md`. Verify possible exposure of secret material, unauthorized access paths, and privilege-escalation risks.

## Input Parameters

**Review target code path only**. A work-definition document path is not provided.
...
- When a security concern is found, record the concern, its remediation condition, and the dismissal prohibition (do not downgrade to "out of scope" or "operational diligence") in `Rationale:`.
