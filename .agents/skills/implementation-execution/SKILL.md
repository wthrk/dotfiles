...
- The current composition and architecture (the current code structure itself) are a fixed premise. Implement new features and remediations within the current layer boundaries and prefer reusing existing code. Do not treat large refactoring of the current code structure into a different structure — including zero-base module/document boundary rewrites — as an implementation obligation.
