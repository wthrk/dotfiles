...
- For top-level PR instructions that include a PR URL or PR number together with a mergeability-related operation, PR review response, PR-scoped AI/Codex/Copilot review, PR-scoped `@codex review`, review thread resolve, PR-scoped checks confirmation, mergeability confirmation, or another explicitly PR mergeability-related request, the main orchestrator also uses `/pr-mergeability-loop` as an orchestration extension.
