...
4. `docs/task-governance/pr-mergeability-loop.md` when the main orchestrator is handling a PR URL or PR number, PR review response, PR-scoped AI/Codex/Copilot review, PR-scoped `@codex review`, review thread resolve, PR-scoped checks confirmation, or mergeability confirmation
...
- For top-level PR instructions that include a PR URL or PR number, PR review response, PR-scoped AI/Codex/Copilot review, PR-scoped `@codex review`, review thread resolve, PR-scoped checks confirmation, or mergeability confirmation, the main orchestrator also uses `/pr-mergeability-loop` as an orchestration extension.
