...
4. `docs/task-governance/pr-mergeability-loop.md` when a PR URL or PR number is specified together with mergeability, checks, review thread, or PR review response request, or when PR review response, PR-scoped AI/Codex/Copilot review, PR-scoped `@codex review`, review thread resolve, PR-scoped checks confirmation, or mergeability confirmation is in scope
...
- Also use `/pr-mergeability-loop` when PR instructions include a PR URL or PR number together with mergeability, checks, review thread, or PR review response request, PR review response, PR-scoped AI/Codex/Copilot review, PR-scoped `@codex review`, review thread resolve, PR-scoped checks confirmation, or mergeability confirmation.
