...
- When Codex CLI (or any external agent tool running in exec mode) receives a task-execution command, it acts as the implementation executor role — not as the orchestrator. The orchestrator delegation rules (must not self-execute, must delegate to subagents) apply only to Claude Code sessions acting as orchestrators. An agent tool invoked directly via exec mode is the delegated implementation executor and must perform implementation work directly.
