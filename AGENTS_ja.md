...
secret-recovery の計画、実装、確認、レビュー、後続対応を伴う作業では、実装担当またはレビュー担当を実際に割り当てて実行できる場合に限り、メインエージェントはオーケストレーション専任とする。現在の実行環境でその担当役割を起動または利用できない場合、現在の実行主体はオーケストレーションで停止したり文書進捗へ逃げたりせず、必要な実装担当へ直ちにフォールバックしなければならない。
...
このリポジトリで作業を始めるときは、他の解釈より先に `docs/README.md`、続いて `docs/secret-recovery/tasks.md` を必ず読む。`docs/secret-recovery/tasks.md` はこのリポジトリでタスクリストとして扱う唯一のファイルである。セッション再開時、コンテキストクリア後、または継続依頼を受けたときは、どの作業を進めるかを質問する前に必ずこのファイルを読んで着手対象を確定する。そのファイルが支配する作業の実装変更やレビューの前に `docs/secret-recovery/implementation-guidelines.md` を読み、固定実装単位、役割分担、実装方針を適用する。
タスク管理の解釈規則（進行依頼全般）は `docs/docs-governance.md` を正本とし、secret-recovery では `docs/secret-recovery/tasks.md` を参照し、進捗変化のたびに同ファイルへ反映する。
...
- secret-recovery の進行依頼では、主対象の判断正本を `docs/secret-recovery/tasks.md` とし、利用者が文書是正そのものを明示的に依頼した場合を除き、この文書の作業項目は実コード差分を主成果物として扱う。
- secret-recovery の進行依頼では、`docs/secret-recovery/tasks.md` の各作業項目にある `対象コードパス` を実装の開始点として扱う。ただしそれは編集上限ではなく、直接必要な呼び出し元、呼び出し先、共有型、port / adapter、対応テストは追ってよい。
- `docs/secret-recovery/tasks.md` の作業項目では、関連する実行パスにコード差分がない限り、文書のみの差分を実装進捗として扱ってはならない。
- secret-recovery の進捗更新では、`文書整合` と `実装` を明示的に分離し、関連する実行パスにコード差分がない場合は `コード差分なし` を記録する。
- `docs/secret-recovery/tasks.md` の作業項目で関連コード差分が存在しない場合、`確認` と `レビュー` は `未着手` で停止し、`実装状態` は `未実装` または `実装中` から前進させず、実装準備完了やレビュー準備完了として扱ってはならない。
- `docs/secret-recovery/tasks.md` の作業項目では、`実装状態` / `確認` / `レビュー` の前進遷移は、同一変更セット内で前提証跡（関連コード差分識別子と必要な確認・レビュー成果物）を同時更新した場合に限って有効とする。
- `docs/secret-recovery/tasks.md` の作業項目では、`コード差分なし` 記録は暫定的な文書記録に限定し、`実装状態` / `確認` / `レビュー` の前進根拠として使用してはならない。
- `docs/secret-recovery/tasks.md` の作業項目では、文書のみの作業を進行依頼への主応答にしてはならない。文書更新は従属成果物であり、作業項目そのものの完了報告には使えない。
- `docs/secret-recovery/tasks.md` の作業項目では、文書修正は「利用者が明示要求した場合」または「実装成果物との整合維持に直接必要な場合」に限って許可し、必ず実コード差分の従属作業として扱う。
