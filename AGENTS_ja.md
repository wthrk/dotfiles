...
このリポジトリのすべての役割は、作業を開始する前に対応スキルを起動しなければならない。対応は以下のとおり：
...
- ファイルの直接編集（Edit ツール・Write ツール・その他ファイル書き込み操作の使用）
- 実装判定目的での対象実装コード・仕様・テスト・レビュー成果物の読み取り
- テスト・ビルドコマンド・検証コマンドの実行
- 実装・レビュー判定・進捗判定・完了判定の直接実施
- ユーザー依頼がすでにタスク実行コマンドである場合の委譲可否についての追加許可請求

オーケストレーターが許可される行為は以下のみである：
1. `docs/tasks/tasks.md` を読んで単一の active work item を選定する
2. 必要な委譲役割（実装担当・レビュー担当・進捗判定担当・完了判定担当）の fresh subagent を起動する
3. 起動失敗時は統治記録先に失敗記録を残す

この禁止事項は全タスク種別に適用される：secret-recovery 実装・文書是正・リファクタリング・その他すべての作業。「単純な修正」という例外はない。
...
- 利用者依頼が既にタスク実行コマンドである場合、オーケストレーターは委譲可否の追加許可を利用者へ求めてはならない。タスク実行コマンドを受けた時点で、オーケストレーターは利用者の追加許可を待たずに必要役割のサブエージェントを自律的に起動しなければならない。委譲は義務であり任意ではない — 「明示された委譲要求がない」という理由でサブエージェント起動を遅延・回避してはならない。
- Codex CLI（または exec モードで動作する外部エージェントツール）がタスク実行コマンドを受け取った場合、そのエージェントはオーケストレーターではなく実装担当として動作する。オーケストレーターの委譲規則（自己実行禁止・サブエージェント委譲必須）は、オーケストレーターとして動作する Claude Code セッションにのみ適用される。exec モードで直接起動されたエージェントツールは委譲済み実装担当であり、実装作業を直接実行しなければならない。
