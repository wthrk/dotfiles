...
fn map_webpki_errors(err: TlsError) -> TlsError {
    // rustls-platform-verifier 0.3.4 depends on rustls-webpki without its `std` feature on
    // Linux/Unix, so `webpki::Error` is not guaranteed to implement `std::error::Error` and
    // cannot be safely downcast here. Preserve the original rustls error instead of attempting
    // the EKU-specific remap so the crate remains compilable under current lockfile resolution.
    err
}
